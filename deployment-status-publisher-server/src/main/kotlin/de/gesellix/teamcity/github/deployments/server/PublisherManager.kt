...
class PublisherManager(extensionsProvider: ExtensionsProvider) {
  ...
  fun createPublisher(buildType: SBuildType, buildFeatureId: String, params: Map<String, String>): GitHubDeploymentsStatusPublisher? {
    ...
    val settings: GitHubDeploymentsStatusPublisherSettings = findSettings(publisherId) ?: return null
  ...
  fun findSettings(publisherId: String): GitHubDeploymentsStatusPublisherSettings? {
    return publisherSettings.extensions
      .stream()
      .filter { s: GitHubDeploymentsStatusPublisherSettings? -> publisherId == s?.getId() }
      .findFirst()
      .orElse(null)
  }
